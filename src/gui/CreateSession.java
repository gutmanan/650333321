...
import init.CreateSessionControll;
...
public class CreateSession extends javax.swing.JPanel {
    ...
    public CreateSession() {
        initComponents();
        for (Map.Entry<String, ArrayList<java.util.Date>> entry : MainClass.getXML().getOccupied().entrySet()) {
            String key = entry.getKey();
            if (key.equals(WindowManager.getTmpArtist().getAlphaCode())) {
                ArrayList<java.util.Date> value = entry.getValue();
                System.out.println("\tConnected users's shows:");
    ...
    private void jComboBox1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jComboBox1ActionPerformed
        ...
        System.out.println("\tStarts at: "+selectedStart.getHours()+":"+selectedStart.getMinutes());
        System.out.println("\tEnds at: "+selectedEnd.getHours()+":"+selectedEnd.getMinutes());
        ...
        for (Map.Entry<Integer, ArrayList<Integer>> entries : studiosRooms.entrySet()) {
            ...
            if (String.valueOf(key).equals(String.valueOf(jComboBox1.getSelectedItem()))) {
                try {
                    System.out.println("\tSelected studio: "+key);
                    model.setRowCount(0);
                    for (Integer room : value) {
                        System.err.println("\tAdding room "+room+" to table");
                        Room tmpRoom = csController.getRoom(key, room);
                        model.addRow(new Object[]{tmpRoom.getRoomNum(),tmpRoom.isRecordingCell(), false});
                        System.err.println("\tAdded room "+tmpRoom.getRoomNum()+"  "+tmpRoom.isRecordingCell()+" to table");
                    }
                    ResultSet rs = MainClass.getDB().query("SELECT Studio.*\n" +
                                                           "FROM Studio\n" +
                                                           "WHERE Studio.studioNumber="+key+";");
                    while (rs.next()) {                        
                        System.out.println("\t\tStudio name: "+rs.getString(2));
                        System.out.println("\t\tStudio description: "+rs.getString(3));
                        System.out.println("\t\tStudio email: "+rs.getString(4));
                        System.out.println("\t\tStudio phone: "+rs.getString(5));
                        System.out.println("\t\tStudio address: "+rs.getString(6));
        ...
        for (Map.Entry<Integer, ArrayList<Integer>> entries : studiosRooms.entrySet()) {
            Integer key = entries.getKey();
            ArrayList<Integer> value =  entries.getValue();
            System.out.println("\tStudio "+key+ " contains:");
